...
public partial class Sugar_Report_rptSaleBillBalance : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (isAuthenticate == "1" || User_Type == "A")
            {
                ...
                pagehead = "Sale Bill Balance Register from " + fromdt + " To " + todt;
                ...
                rprt1.Load(Server.MapPath("CrySaleBillBalance.rpt"));
                rprt1.SetDataSource(dt);
                CrySaleBillBalance.ReportSource = rprt1;
    ...
    private DataTable GetData(string fromdt, string todt)
    {
        ...
        using (SqlConnection con = new SqlConnection(strcon))
        {
            ...
            if ( Accode!="0")
            {
                cmd = new SqlCommand("select * from qrysalebillbalance where Ac_Code=" + Accode + " and doc_date between '" + datefrom + "' and '" + dateto + "' and Company_Code='"
               + Convert.ToInt32(Session["Company_Code"].ToString()) + "'", con);
            }
            else
            {
                cmd = new SqlCommand("select * from qrysalebillbalance where doc_date between '" + datefrom + "' and '" + dateto + "' and Company_Code='"
               + Convert.ToInt32(Session["Company_Code"].ToString()) + "'", con);
    ...
    protected void Page_Unload(object sender, EventArgs e)
    {
        ...
        //rprt1.Dispose();
        //GC.Collect();
        this.CrySaleBillBalance.ReportSource = null;

        CrySaleBillBalance.Dispose();
